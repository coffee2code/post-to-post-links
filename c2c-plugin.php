...
if ( ! class_exists( 'c2c_EasyPostToPostLinks_Plugin_047' ) ) :

abstract class c2c_EasyPostToPostLinks_Plugin_047 {
	...
	protected function __construct( $version, $id_base, $author_prefix, $file, $plugin_options = array() ) {
		...
		if ( ! file_exists( $file ) ) {
			die( sprintf( __( 'Invalid file specified for C2C_Plugin: %s', 'easy-post-to-post-links' ), $file ) );
		}
	 ...
	 * @since 036
	 */
	public function __clone() { _doing_it_wrong( __FUNCTION__, __( 'Something went wrong.', 'easy-post-to-post-links' ), '036' ); }
	 ...
	 * @since 036
	 */
	public function __wakeup() { _doing_it_wrong( __FUNCTION__, __( 'Something went wrong.', 'easy-post-to-post-links' ), '036' ); }
	...
	public function options_page_description( $localized_heading_text = '' ) {
		...
		if ( ! $this->disable_contextual_help ) {
			echo '<p class="see-help">' . __( 'See the "Help" link to the top-right of the page for more help.', 'easy-post-to-post-links' ) . "</p>\n";
		}
	}
	...
	public function sanitize_inputs( $inputs ) {
		...
		if ( isset( $_POST['Reset'] ) ) {
			...
			add_settings_error( 'general', 'settings_reset', __( 'Settings reset.', 'easy-post-to-post-links' ), 'updated' );
		} else {
			...
			foreach ( $option_names as $opt ) {
				if ( !isset( $inputs[ $opt ] ) ) {
					if ( $this->config[ $opt ]['input'] == 'checkbox' ) {
						...
						$msg = sprintf( __( 'A value is required for: "%s"', 'easy-post-to-post-links' ), $this->config[ $opt ]['label'] );
						...
						switch ( $this->config[ $opt ]['datatype'] ) {
							...
							case 'int':
								if ( ! empty( $val ) && ( ! is_numeric( $val ) || ( intval( $val ) != round( $val ) ) ) ) {
									$msg = sprintf( __( 'Expected integer value for: %s', 'easy-post-to-post-links' ), $this->config[ $opt ]['label'] );
									$error = true;
									$val = '';
								}
								break;
	...
	public function verify_config() {
		...
		foreach ( $this->required_config as $config ) {
			if ( empty( $this->$config ) ) {
				die( sprintf( __( "The plugin configuration option '%s' must be supplied.", 'easy-post-to-post-links' ), $config ) );
			}
		}
	...
	protected function load_textdomain() {
		load_plugin_textdomain( 'easy-post-to-post-links' );
	}
	...
	public function contextual_help( $contextual_help, $screen_id, $screen = null ) {
		...
		$help = '<h3>' . __( 'More Plugin Help', 'easy-post-to-post-links' ) . '</h3>';
		$help .= '<p class="more-help">';
		$help .= '<a title="' . esc_attr( sprintf( __( 'More information about %1$s %2$s', 'easy-post-to-post-links' ), $this->name, $this->version ) ) .
			'" class="thickbox" href="' . $help_url . '">' . __( 'Click for more help on this plugin', 'easy-post-to-post-links' ) . '</a>' .
			__( ' (especially check out the "Other Notes" tab, if present)', 'easy-post-to-post-links' );
		$help .= ".</p>\n";
		return $help;
	}
...
HTML;
	...
	public function help_tabs_content( $screen ) {
		$screen->add_help_tab( array(
			...
			'title'   => __( 'More Help', 'easy-post-to-post-links' ),
	...
	public function plugin_action_links( $action_links ) {
		$settings_link = '<a href="' . $this->settings_page . '.php?page='.$this->plugin_basename.'">' . __( 'Settings', 'easy-post-to-post-links' ) . '</a>';
	...
	public function donate_link( $links, $file ) {
		if ( $file == $this->plugin_basename ) {
			...
			$donation_url .= urlencode( sprintf( __( 'Donation for coffee2code plugin: %s', 'easy-post-to-post-links' ), $this->name ) );
			$title         = __( 'Coffee fuels my coding.', 'easy-post-to-post-links' );
			$links[] = '<a href="' . esc_url( $donation_url ) . '" title="' . esc_attr( $title ) . '">' . __( 'Donate', 'easy-post-to-post-links' ) . '</a>';
		}
		return $links;
	}
...
//				echo '<div class="hr">&nbsp;</div>' . "\n";
	...
	public function options_page() {
		...
		echo '<input type="submit" name="Submit" class="button-primary" value="' . esc_attr__( 'Save Changes', 'easy-post-to-post-links' ) . '" />' . "\n";
		echo '<input type="submit" name="Reset" class="button" value="' . esc_attr__( 'Reset Settings', 'easy-post-to-post-links' ) . '" />' . "\n";
		echo '</form>' . "\n";

		do_action( $this->get_hook( 'after_settings_form' ), $this );

		echo '<div id="c2c" class="wrap"><div>' . "\n";
		$c2c = '<a href="http://coffee2code.com" title="coffee2code.com">' . __( 'Scott Reilly, aka coffee2code', 'easy-post-to-post-links' ) . '</a>';
		echo sprintf( __( 'This plugin brought to you by %s.', 'easy-post-to-post-links' ), $c2c );
		echo '<span><a href="https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=6ARCFJ9TX3522" title="' . esc_attr__( 'Please consider a donation', 'easy-post-to-post-links' ) . '">' .
		__( 'Did you find this plugin useful?', 'easy-post-to-post-links' ) . '</a></span>';
		echo '</div>' . "\n";
